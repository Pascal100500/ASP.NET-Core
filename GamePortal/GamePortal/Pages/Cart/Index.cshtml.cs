    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostCheckout()
        {
            ...
            try
            {
                ...
                "Успешная покупка пользователем: {User} Игр куплено: {Count} всего: {Total}  IP: {IP}",
                userId,
                activeItems.Count,
            ...
            catch (Exception ex)
            {
                ...
                //логирование неудачной оплаты
                using (LogContext.PushProperty("LogType", "User"))
                {
                    _logger.LogError(ex, "Ошибка при оформлении оплаты пользоателем {User} IP: {IP}", userId, ip);
                }
            }
